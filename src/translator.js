...
export class NahuatlTranslator {
  ...
  generateEnglishTranslation(parsedMorphemes) {
    ...
    const allNounStemsDetails = []; // To collect all noun stems for compounding
    ...
    const hasPluralSuffix = parsedMorphemes.some((m) => m.details.type === 'suffix' && pluralSuffixMorphemes.has(m.morpheme));
    const one = 1;
    const two = 2;
    ...
    if (isLliParticiple) {
      ...
      if (allNounStemsDetails.length > 0 && mainVerbStemDetails) {
        ...
        if (hasPluralSuffix) {
          ...
          if (agentForm.endsWith('y') && !['a', 'e', 'i', 'o', 'u'].includes(agentForm[agentForm.length - two])) {
            pluralAgent = agentForm.slice(0, -one) + 'ies';
        ...
        if (hasPluralSuffix) {
          // Pluralize the agent form
          if (agentForm.endsWith('y') && !['a', 'e', 'i', 'o', 'u'].includes(agentForm[agentForm.length - two])) {
            agentForm = agentForm.slice(0, -one) + 'ies';
          } else if (agentForm.endsWith('s') || agentForm.endsWith('sh') || agentForm.endsWith('ch') ||
                               agentForm.endsWith('x') || agentForm.endsWith('z')) {
      ...
      const getProgressiveForm = (verbStem) => {
        ...
        const english = verbStem.english;
        if (english.endsWith('e')) {
          return english.slice(0, -one) + 'ing';
        } else {
          return english + 'ing';
        }
      };
      ...
      let nominalizedForm = (mainVerbStemDetails ? mainVerbStemDetails.english : '') + otherNominalizingSuffixDetails.englishSuffix;
      if (hasPluralSuffix) {
    ...
    if (!subjectPrefixDetails && !imperativePrefixDetails) {
      ...
      const subjectString = subjectPrefixDetails.english;
      const verbOrNounString = coreEnglishString;
